...
public:
        ...
        void clear() {
                logoffset=0;
                lastlog=0;
                for(minmaxindex=0;minmaxindex<entries();minmaxindex++)
                        minmax(y[minmaxindex]=LOGNAN);
        }

        void reminmax() {
                for(minmaxindex=0;minmaxindex<entries();minmaxindex++)
                        minmax(y[minmaxindex]);
        }
        ...
        void minmax(ytype t) {
                ...
                if (t==LOGNAN) return;
                if (tmin==LOGNAN) tmin=t; else if (t<tmin) tmin=t;
                if (tmax==LOGNAN) tmax=t; else if (t>tmax) tmax=t;
        ...
        int update(String result) {
                ...
                while(result.length() > 0) {
                        ...
                        if (type.equals("seconds")) {
                                captime=nextdouble(line,&fields); /* drops decimal (floor) */
                                /* ignore multiple capture times */
                                continue;
                        }
                        if (type.equals("degC")) {
                                for(int i=0; i<probes(); i++)
                                        val[i].degC=nextdouble(line,&fields);
                                continue;
                        }
                        if (type.equals("%rh")) {
                                for(int i=0; i<probes(); i++)
                                        val[i].relh=nextdouble(line,&fields);
                                continue;
                        }
                        if (type.equals("hPa")) {
                                for(int i=0; i<probes(); i++)
                                        val[i].hPa=nextdouble(line,&fields);
                                continue;
                        }
                        if (type.equals("state")) {
                                for(int i=0; i<probes(); i++)
                                        val[i].state=nextdouble(line,&fields);
                                continue;
                        }
                }
                checkalert();
...
public:
        ...
        void graph(OLEDDisplay *display, int16_t x, int16_t y) {
                ...
                int tspacing;
                if      (tdivision<=   60) {tdivision=   60; tspacing=16;}
                else if (tdivision<= 5*60) {tdivision= 5*60; tspacing=16;}
                else if (tdivision<=10*60) {tdivision=10*60; tspacing=8;}
                else if (tdivision<=30*60) {tdivision=30*60; tspacing=8;}
                else                       {tdivision=60*60; tspacing=4;}
                ...
                for(int temp=int(ymin/ydivision-1)*ydivision;temp<=int(1+ymax/ydivision)*ydivision;temp+=ydivision) {
                        int yy=SCALEY(temp);
                        int xspacing;
                        if      (temp%10==0) xspacing=2;
                        else if (temp%5==0)  xspacing=4;
                        else if (temp%2==0)  xspacing=8;
                        else                 xspacing=16;
                        if (yy<0 || yy>=height) continue;
                        display->drawString(x,y+SCALEY(temp)-5,String(temp));
                        for(int xx=0; xx<width; xx+=xspacing)
                                display->setPixel(x+xx,y+yy);
                }
                for(time_t tt=tmarker;tt>tmin;tt-=tdivision) {
                        int xx=SCALEX(tt);
                        if (xx<0 || xx>=width) continue;
                        for(int yy=0;yy<height;yy+=tspacing)
                                display->setPixel(x+xx,y+yy);
                }
...
static int getdata() {
        ...
        pv.clear();
        if (degc!=bad) {pv.degC=degc; readings++;}
        if (rh!=bad)   {pv.relh=rh; readings++;}
...
static String jsonkeyval(String key,int val,bool comma=true) {
...
void drawprobe0(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {frames.probe(display,x,y,0); checkwork(state);}
void drawprobe1(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {frames.probe(display,x,y,1); checkwork(state);}
void drawprobe2(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {frames.probe(display,x,y,2); checkwork(state);}
void drawprobe4(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {frames.probe(display,x,y,4); checkwork(state);}
void drawgraph(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y)  {frames.graph(display,x,y);   checkwork(state);}
