...
/* To program, typically requires installing the Silicon Labs CP2102 USB driver,
 * and briefly holding button 2 (the one not marked EN) while programmer is
 * attempting to program the device.
 */
...
#if defined(USE_PATCHED) && USE_PATCHED
#include "../patched.hpp"
byte const *write_buffer = static_cast<byte const *>(patched_hpp);
size_t write_buffer_size = sizeof(patched_hpp);
#else
#include "../donor.hpp"
byte const *write_buffer = static_cast<byte const *>(donor_hpp);
size_t write_buffer_size = sizeof(donor_hpp);
#endif
